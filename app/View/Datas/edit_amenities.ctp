   ...
   // pr($lead);
	...
	$class='extramarLeft';
		...
		?>
		    ...
		    <input type="text" class="form-control add <?php echo $class;?>" name="data[LookupValueAmenitie][amenity_name][]" data-required="true" value="<?php echo $amenities['LookupValueAmenitie']['amenity_name'];?>" >
		    <?php if($i == 1){?>
		...
		?>
		<span class="removeVar" style="display: inline-block;height: 14px;margin: 10px 0 0 10px;text-indent: -9999em;vertical-align: middle;width: 14px;background: url(../../img/remove-icon.png) no-repeat 0 0; float: right; position: relative;
top: -32px; right: -27.5%;">Remove Variable</span>
		<?php }?>
		</div>
